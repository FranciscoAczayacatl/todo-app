...
taskArea.addEventListener("click", (e) => {
  ...
  if(e.target.classList.contains("btn-complete")){
    const taskId = e.target.getAttribute("id");
    //encotrar el elemanto que tenga el id que acabo de obtener
    const index=tasksArray.findIndex((task)=>task.id==Number(taskId));
   //en el arreglo modifico la propiedad compete del elemento encontrado
...
addButton.addEventListener("click", (e) => {
  ...
  tasksArray.push({ title, description, complete: false, id: idConter});
  localStorage.setItem("tasks", JSON.stringify(tasksArray));
  clearInputs();
  createTask();
  idConter ++;
    ...
    return `
      <div class="task-item">
        ...
        <div class="task-buttons">
          <button class="btn btn-complete ${task.complete?"hide":""}" id=${task.id}>Competar</button>
